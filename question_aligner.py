...
class QuestionNumberAligner:
    ...
    def match_with_gemini(self, df: pd.DataFrame, questions: List[Question]) -> Dict[str, str]:
        ...
        2.  Multi-select questions in the dataset often appear as multiple columns with the same base text followed by an option (e.g., "Q7. Which do you own? - Car", "Q7. Which do you own? - Boat"). All of these columns should map to the same question number.
        3.  **Crucially**, if a `column_header` does not correspond to any of the `survey_questions` (e.g., it is a programming note, an open-ended response field not in the PDF, or metadata), you MUST assign it the value "UNMATCHED". Do not force a match.
        4.  Your final output must be a single, valid JSON object where the keys are the exact original `column_headers` and the values are the corresponding question numbers (as strings) or the string "UNMATCHED".
